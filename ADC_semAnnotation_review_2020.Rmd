...
A goal is to assess if these unannotated attributes are (a) terms that datateam members are intentionally skipping for sake of time (e.g. "less-important" attributes), (b) terms that really should have an annotation and got skipped accidentally, or (c) terms that got skipped because there is currently no appropriate semantic annotation to describe them.
...
I've manually assigned terms to some general categories (see legend) -- location terms (colored green; e.g. "position", "latitude", "top", "site", etc.), temporal terms (colored pink; e.g. "data", "phase"), and QC/Confidence terms (colored yellow) are likely being intentionally skipped for sake of time (see instructions in [Data Team Training Part 4.8.2 #1](https://nceas.github.io/datateam-training/training/editing-eml.html#semantic-annotations)), whereas measurements (colored orange; e.g. "depth", "height") and environmental materials (colored blue; e.g. "soil", "snow", "ice") likely have an appropriate annotation match (ECSO & ENVO) and could be annotated. 
...
datatable(attributeNameIndivTokens,
          ...
          caption = htmltools::tags$caption(
            ...
            htmltools::em('Most common indiviudal attributeName tokens (terms) left unannotated. In addition to the total counts of each token, the number of unique package identifiers that that term is found in & the number of unique authors who used that term are also included. These data may be important for determining which tokens are (i) being broadly used many times (e.g. "date") used many times (ii) being used many times but in just a few different data packages (e.g. "position") or (iii) used many times by only one author across many data packages (possibly indicative on nested datapackages; e.g. "habitat_type)')),
          filter = 'top', options = list(pageLength = 5, autoWidth = TRUE))
```
