...
annotate_single_otherEntity_single_attribute <- function(doc, current_attribut_id, attributeName_subset){
 
  # add attribute id to metadata
  doc$dataset$otherEntity$attributeList$attribute$id <- current_attribute_id
  ...
  doc$dataset$otherEntity$attributeList$attribute$annotation$valueURI <- list(label = current_label,
