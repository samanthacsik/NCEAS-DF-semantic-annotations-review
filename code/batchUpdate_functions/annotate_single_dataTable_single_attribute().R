annotate_single_dataTable_single_attribute <- function(doc, current_attribute_id, attributeName_subset){
  # add attribute id to metadata
  doc$dataset$dataTable$attributeList$attribute$id <- current_attribute_id
  ...
  doc$dataset$dataTable$attributeList$attribute$annotation$propertyURI <- list(label = "contains meausurements of",
  ...
  doc$dataset$dataTable$attributeList$attribute$annotation$valueURI <- list(label = current_label,
