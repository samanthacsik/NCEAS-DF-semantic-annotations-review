#' Add semantic annotations to EML metadata containing a single, unpacked entity
#'
...
annotate_attributes_unpackedEntity <- function(doc, dataTable_or_otherEntity, all_entities_byType, is_single_entity){
  ...
  current_eml_entity <- all_entities_byType # to keep names consistent
  ...
  # ---------------------------------------------------------------------------------------------
  # if the current (unpacked) entity contains more than 1 attribute (i.e. `is_single_attribute` == FALSE), annotate the attributes using the following methods
  # ---------------------------------------------------------------------------------------------
    ...
    for(eml_att_num in 1:num_attributes_in_eml_entity){
      ...
      if(dataTable_or_otherEntity == "dataTable"){
        ...
        if(is_single_entity){
          ...
          current_attribute_id <- build_attributeID_unpackedEntity(eml_att_num, entity_path, current_eml_entity) 
          ...
          stop("The dataTable you are trying to process is one of multiple dataTables (i.e. it is PACKED). Double check which version of build_attributeID() you are using.")
        ...
        if(is_single_entity){
          ...
          stop("The otherEntity you are trying to process is one of multiple otherEntities (i.e. it is PACKED). Double check which version of build_attributeID() you are using.")
  ...
  # ---------------------------------------------------------------------------------------------
  # if the current (unpacked) entity contains only a single (unpacked) attribute, annotate using the following methods
  # ---------------------------------------------------------------------------------------------
  ...
  if(isTRUE(is_single_attribute)){
    ...
    if(dataTable_or_otherEntity == "dataTable"){
      ...
      # if the current entity is not a 'dataTable' or 'otherEntity' throw an error
    } else{
      stop("The current entity you are trying to process is not a dataTable or an otherEntity")
    }
  }
  ...
  message("Processed single, unpacked ", dataTable_or_otherEntity, " | ",  current_entityName_from_eml)
